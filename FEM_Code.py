...
def computeSolution(target_fun, domain, degree, solution_basis):
    ...
    xmin = domain[0]
    xmax = domain[1]
    ien_array, node_coords = mesh.generateMesh(xmin, xmax, degree)
...
def change_of_basis(domain_old, domain_new, x_old):
    a = domain_old[0]
    b = domain_old[1]
    c = domain_new[0]
    d = domain_new[1]
...
def assembleForceVector(target_fun, ien_array, node_coords, solution_basis):
    domain_old = [node_coords[0,0],node_coords,[-1,-1]]
    domain_new = [-1,1]
    jacob = jacobian(domain_old,domain_new)
    degree = len(ien_array[0])-1
    n = numpy.ceil((2*degree+1)/2)
    qp, w = quad.computeGaussLegendreQuadrature(n)
    F = numpy.zeros(degree+1)
    
    for i in range(degree+1):
        for q in range(degree+1):
            N_A = basis.evaluateBernsteinBasis1D(qp[q], degree, i)
            F += N_A * target_fun(change_of_basis(domain_old, domain_new, qp[q])) * w[q] * jacob
    return F
